...
function transformToLis(obj){
  // Solution code here...
  let localArr=[];
  let transform=Object.entries(obj);
  transform.forEach(([key,value])=>{
    localArr.push(`<li>${key}: ${value}</li>`);
  });
  return localArr;
}
/* ------------------------------------------------------------------------------------------------
...
const totalSum = (input) => {
  // Solution code here...
  let result = input.reduce((pre,next )=> {
    return pre+=next.reduce((acc,cur)=> acc+=cur); },0);
  return result;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    let newArr = input[i];
    localArr.push(newArr.filter(num => ((num%5===0) && typeof (num) === 'number')).map(num => Math.pow(2, num)));
  }
  return localArr;
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.filter( char => char.height).reduce((a, b) => (b.height > a.height ? b : a));

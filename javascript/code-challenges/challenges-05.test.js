...
const toLastNames = people => {
  ...
  let newList = people.map(e => `${e.firstName} ${e.lastName}`);
  return newList;
...
const addPurchases = (arr) => {
  ...
  let localArr=arr.reduce((previousValue, currentValue) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let newArr = [];
  newArr= arr.reduce((previousValue, currentValue, index)=>{
    previousValue[index] = currentValue.name;
...
const reversedString = (str) => {
  // Solution code here...
  let reversedStr;
  reversedStr = str.split('').reduce((pre,current)=>{
  return current+pre;
},'')
return reversedStr;
};
